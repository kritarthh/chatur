defmodule Mouse do
  ...
  def move(x, y, relative \\ true) do
    Logger.debug("Moving mouse to (#{x},#{y}) #{if relative, do: "relatively", else: "absolutely"}")
  ...
  def approach(x, y, type \\ :nonlinear, r \\ 0.05, mr \\ 20) do
    ...
    ratex = if abs(rdx) < 1, do: dx, else: if abs(rdx) > mr, do: round(mr * (rdx/abs(rdx))), else: rdx
    ratey = if abs(rdy) < 1, do: dy, else: if abs(rdy) > mr, do: round(mr * (rdy/abs(rdy))), else: rdy
